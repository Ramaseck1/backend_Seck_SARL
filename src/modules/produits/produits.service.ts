 ...
 */
export async function creerProduit(input: CreerProduitInput) {
  const quantite = input.quantiteStock ?? 0;

  const dateAchat = input.dateAchat ?? new Date();

  const totalInvesti =
    quantite * input.prixAchatUnitaire;
...
export async function ajusterStock(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    let totalInvesti =
      Number(produit.totalInvesti);

    let nouveauPrixAchat =
    ...
    if (delta > 0) {
      ...
      if (
        ...
        throw new AppError(
          "Le prix d'achat est obligatoire pour une entrée de stock.",
          400
        );
      }

      const montantAchat =
        delta * prixAchatUnitaire;
 ...
 * Produits en alerte.
 */
export async function produitsEnAlerte(
  activiteId: string
) {
 ...
 */
export async function obtenirBilanProduit(
  produitId: string
) {
  ...
  const totalInvesti =
    produit.achats.reduce(
      (total, achat) =>
        total +
        Number(achat.montantTotal),
      0
    );
  ...
  const montantTotalVendu =
    produit.ventes.reduce(
      (total, vente) =>
        total +
        Number(vente.montant),
      0
    );
  ...
  const totalCredit =
    ventesCredit.reduce(
      (total, vente) =>
        total +
        Number(vente.credit?.montantInitial ?? 0),
      0
    );

  const totalCreditRembourse =
    ventesCredit.reduce(
      (total, vente) =>
        total +
        Number(vente.credit?.montantRembourse ?? 0),
      0
    );

  const totalCreditRestant =
    ventesCredit.reduce(
      (total, vente) =>
        total +
        Number(vente.credit?.montantRestant ?? 0),
      0
    );
  ...
  const clientsNonPayes =
    ventesCredit
      .filter(
        (vente) =>
          Number(
            vente.credit?.montantRestant ?? 0
          ) > 0
      )
      .map((vente) => ({
        venteId: vente.id,
        ...
        montantInitial:
          Number(
            vente.credit?.montantInitial ?? 0
          ),

        montantRembourse:
          Number(
            vente.credit?.montantRembourse ?? 0
          ),
  ...
  return {
    ...
    produit: {
      ...
      dateFinSession:
        produit.dateFinSession,

      sessionTerminee:
        produit.sessionTerminee,
    ...
    ventes: {
      ...
      nombreVentes:
        produit.ventes.length,

      details:
        produit.ventes.map((vente) => ({
          id: vente.id,
          ...
          credit: vente.credit
            ? {
                montantInitial:
                  Number(
                    vente.credit.montantInitial
                  ),

                montantRembourse:
                  Number(
                    vente.credit.montantRembourse
                  ),

                montantRestant:
                  Number(
                    vente.credit.montantRestant
                  ),
