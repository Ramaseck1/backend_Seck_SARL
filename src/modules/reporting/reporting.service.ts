...
export async function rapportJournalier(activiteId: string, dateStr?: string) {
  ...
  const [ventes, remboursements, mouvements] = await Promise.all([
    prisma.vente.findMany({ where: { activiteId, date: { gte: debut, lte: fin } } }),
    prisma.remboursement.findMany({
      where: { credit: { activiteId }, date: { gte: debut, lte: fin } },
    }),
    prisma.mouvementCaisse.findMany({ where: { activiteId, date: { gte: debut, lte: fin } } }),
  ]);
...
export async function rapportConsolide() {
